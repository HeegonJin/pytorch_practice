...
def mutate_tensor(x, indices, values):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  for items in (zip(indices, values)):
      row, col = items[0][0], items[0][1]
      x[row][col] = items[1]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x = [10 * i for i in range(start//10+1, stop//10+1)]
...
def make_one_hot(x):
  ...
  # row_mask = [i for i in range(len(x))]
  # y[row_mask, x] = 1
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  _, M, P = y.shape[0], y.shape[1], y.shape[2]
  if use_loop is True:
        z_list = []
        for batch in range(B):
            z_list.append(torch.mm(x[batch], y[batch]))
...
def normalize_columns(x):
  ...
  x_col_mean = x.sum(dim=0) / M
  x_col_std = torch.sqrt(torch.sum((x-x_col_mean)**2, dim=0)/(M-1))
  y = (x - x_col_mean) / x_col_std
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
